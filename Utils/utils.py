...
def calculateDuration(df):
    df['Duration'] = (df['CompleteTimestamp'] - df['CompleteTimestamp'].shift(1)).fillna(pd.Timedelta(seconds=0))
...
def plotReconstructionError(error_df, variable='Activity'):
    ...
    plt.title('Reconstructionsfehler normaler Events')
    
    #anomalous cases
    plt.subplot(122)
    anomaly_error_df.hist()
    plt.title('Reconstructionsfehler fehlerhafter Events')
